...
#include "legate_sparse/partition/fast_image_partition.h"
#include "legate_sparse/partition/fast_image_partition_template.inl"
...
struct FastImageRangeImplBody<VariantKind::GPU, INDEX_CODE> {
                  ...
                  const Rect<1>& bounds)
  {
    ...
    thrust::pair<const INDEX_TY*, const INDEX_TY*> result = thrust::minmax_element(
      thrust_exec_policy, in_crd.ptr(bounds.lo[0]), in_crd.ptr(bounds.hi[0]) + 1);
